...
it('should return correct position', () => {
  const newPosition1 = findNextPosition({ x: 6, y: 14 }, 3);
  const newPosition2 = findNextPosition({ x: 3, y: 8 }, 3);
  const newPosition3 = findNextPosition({ x: 0, y: 6 }, 3);
...
it('should handle corners', () => {
  const newPosition1 = findNextPosition({ x: 6, y: 10 }, 3);
